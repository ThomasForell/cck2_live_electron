        ...
        <Box sx={{ flexGrow: 1, bgcolor: 'background.paper', display: 'flex', height: "100%" }}>
            <Stack spacing={4} direction="column">
                <Stack spacing={25} direction="row" justifyContent="space-between">
                    ...
                    <Button onClick={() => { 
                        reset(watchedValues); 
                        (window as any).electronAPI.saveSingleSetup(watchedValues); }} disabled={!isDirty} variant="contained">Speichern</Button>
                </Stack>
                <Stack spacing={2} direction="column">
                    <Stack spacing={4} direction="row">
                        <TextField key="single_match_day" label="Spieltag" variant={variant} type="number" InputLabelProps={{shrink: true,}}
                            {...register("match_day")} defaultValue={1} />
                        <TextField key="single_num_match_days" label="Anzahl Spieltage" variant={variant} type="number"
                            {...register("num_match_days")} defaultValue={2} />
                    </Stack>
                    <TextField key="single_cck2_output_files" label="CCK2 Ausgabedateien, mehrere Dateien mit Komma trennen" variant={variant}  
                        {...register("cck2_output_files")} defaultValue="result.json" />
                    <DirectorySelectorElectron key="tab_single_data_dse" register={register} registerName="data_path" setValue={setValue} getValues={getValues}
                        defaultValue="c:\\users\\[Benutzername]\\Documents\\Veranstaltung" label="Datenverzeichnis – Ein- und Ausgabe für das Turnier"/>
                    <TextField key="single_player_data" label="Spielerliste (Spieler ID,Name,Mannschaft,Gruppe,Kommentar) im CSV-Format" variant={variant} 
                    ...
                    <Stack spacing={2} direction="row">
                        <Button variant="contained" onClick={() => { setActive(false); (window as any).electronAPI.singleProcessingStop(); }} 
                            disabled={!active}>Ausgabe Pausieren</Button>
                        <Button variant="contained" onClick={() => { setActive(true); (window as any).electronAPI.singleProcessingStart(); }} 
